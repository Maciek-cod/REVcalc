...
When the user uploads the file, all transactions are analysed and added to the "Transaction" table, and if the transaction is a sales transaction, additional rows are added to the "Sell_detail" table where the links to the purchase transactions are stored. e.g. profit on each share sale by subtracting sold share price from the first share bought.  
